...
import type { Field, PayloadRequest, ReplaceAny, RequestContext, RichTextField, SanitizedConfig, ValidateOptions } from 'payload';
import type React from 'react';
import type { ServerEditorConfig } from '../lexical/config/types.js';
import type { AdapterProps } from '../types.js';
import type { HTMLConverter } from './converters/html/converter/types.js';
import type { ClientComponentProps } from './typesClient.js';
export type PopulationPromise<T extends SerializedLexicalNode = SerializedLexicalNode> = ({ context, currentDepth, depth, draft, editorPopulationPromises, field, fieldPromises, findMany, flattenLocales, node, overrideAccess, populationPromises, req, showHiddenFields, siblingDoc, }: {
...
export type FeatureProviderProviderServer<UnSanitizedServerFeatureProps = undefined, ServerFeatureProps = UnSanitizedServerFeatureProps, ClientFeatureProps = undefined> = (props?: UnSanitizedServerFeatureProps) => FeatureProviderServer<UnSanitizedServerFeatureProps, ServerFeatureProps, ClientFeatureProps>;
export type FeatureProviderServer<UnSanitizedServerFeatureProps = undefined, ServerFeatureProps = UnSanitizedServerFeatureProps, ClientFeatureProps = undefined> = {
    ...
    }) => Promise<ServerFeature<ServerFeatureProps, ClientFeatureProps>> | ServerFeature<ServerFeatureProps, ClientFeatureProps>) | ServerFeature<ServerFeatureProps, ClientFeatureProps>;
    key: string;
...
export type AfterReadNodeHook<T extends SerializedLexicalNode> = (args: AfterReadNodeHookArgs<T> & BaseNodeHookArgs<T>) => Promise<T> | T;
...
export type NodeWithHooks<T extends LexicalNode = any> = {
     ...
     */
    graphQLPopulationPromises?: Array<PopulationPromise<ReturnType<ReplaceAny<T, LexicalNode>['exportJSON']>>>;
    /**
...
export type ResolvedServerFeature<ServerProps, ClientFeatureProps> = {
    order: number;
} & Required<Pick<FeatureProviderServer<ServerProps, ClientFeatureProps>, 'dependencies' | 'dependenciesPriority' | 'dependenciesSoft' | 'key'>> & ServerFeature<ServerProps, ClientFeatureProps>;
...
export type SanitizedServerFeatures = {
    ...
    generatedTypes: {
        modifyOutputSchemas: Array<({ collectionIDFieldTypes, config, currentSchema, field, interfaceNameDefinitions, isRequired, }: {
            collectionIDFieldTypes: {
                [key: string]: 'number' | 'string';
            };
            config?: SanitizedConfig;
    ...
    getSubFieldsData?: Map<string, (args: {
        node: SerializedLexicalNode;
        req: PayloadRequest;
    }) => Record<string, unknown>>;
...
//# sourceMappingURL=typesServer.d.ts.map