...
export const endpointsSchema = joi.alternatives().try(joi.array().items(joi.object({
...
export default joi.object({
    admin: joi.object({
        ...
        components: joi.object().keys({
            ...
            views: joi.alternatives().try(joi.object({
    ...
    bin: joi.array().items(joi.object().keys({
        key: joi.string(),
    ...
    editor: joi.object().optional().keys({
    ...
    secret: joi.string(),
    serverURL: joi.string().uri().allow('').custom((value, helper)=>{
    ...
    typescript: joi.object({
        ...
        declare: joi.alternatives().try(joi.boolean(), joi.object({
            ignoreTSError: joi.boolean()
        })),
