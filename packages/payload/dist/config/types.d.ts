import type { DefaultTranslationsObject, I18nClient, I18nOptions, TFunction } from '@payloadcms/translations';
...
import type { AfterErrorHook, Collection, CollectionConfig, SanitizedCollectionConfig } from '../collections/config/types.js';
...
export type GeneratePreviewURL = (doc: Record<string, unknown>, options: GeneratePreviewURLOptions) => Promise<null | string> | null | string;
...
export type GraphQLExtension = (graphQL: typeof GraphQL, context: {
...
export declare const serverProps: (keyof ServerProps)[];
export type CustomComponent<TAdditionalProps extends any = any> = React.ComponentType<Partial<ServerProps> & TAdditionalProps>;
export type Locale = {
    /**
 ...
 */
export type LocalizationConfig = Prettify<LocalizationConfigWithLabels | LocalizationConfigWithNoLabels>;
export type LabelFunction = ({ t }: {
    t: TFunction;
}) => string;
export type SharpDependency = (input?: ArrayBuffer | Buffer | Float32Array | Float64Array | Int8Array | Int16Array | Int32Array | Uint8Array | Uint8ClampedArray | Uint16Array | Uint32Array | string, options?: sharp.SharpOptions) => sharp.Sharp;
...
} & Omit<DeepRequired<Config>, 'collections' | 'editor' | 'endpoint' | 'globals' | 'i18n' | 'localization' | 'upload'>;
...
export type EntityDescriptionFunction = ({ t }: {
    t: TFunction;
}) => string;
