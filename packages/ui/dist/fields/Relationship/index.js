...
const _RelationshipField = (props)=>{
    const { name, CustomDescription, CustomError, CustomLabel, allowCreate = true, className, descriptionProps, errorProps, hasMany, isSortable = true, label, labelProps, path: pathFromProps, readOnly: readOnlyFromProps, relationTo, required, sortOptions, style, validate, width } = props;
    ...
    const getResults = useCallback(async ({ lastFullyLoadedRelation: lastFullyLoadedRelationArg, lastLoadedPage: lastLoadedPageArg, onSuccess, search: searchArg, sort, value: valueArg })=>{
        if (!permissions) {
            return;
        }
                        ...
                        children: [
                            /*#__PURE__*/ _jsx(ReactSelect, {
                                ...
                                getOptionValue: (option)=>{
                                    ...
                                    return hasMany && Array.isArray(relationTo) ? `${option.relationTo}_${option.value}` : option.value;
                                },
                                isLoading: isLoading,
                                isMulti: hasMany,
