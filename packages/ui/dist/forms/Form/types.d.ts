...
export type FieldAction = ADD_ROW | ADD_SERVER_ERRORS | DUPLICATE_ROW | MODIFY_CONDITION | MOVE_ROW | REMOVE | REMOVE_ROW | REPLACE_ROW | REPLACE_STATE | SET_ALL_ROWS_COLLAPSED | SET_ROW_COLLAPSED | UPDATE;
export type FormFieldsContext = [FormState, Dispatch<FieldAction>];
export type Context = {
    addFieldRow: ({ data, path, rowIndex, schemaPath, }: {
        data?: Data;
        path: string;
        rowIndex?: number;
        schemaPath: string;
    ...
    replaceFieldRow: ({ data, path, rowIndex, schemaPath, }: {
        data?: Data;
        path: string;
        rowIndex: number;
        schemaPath: string;
