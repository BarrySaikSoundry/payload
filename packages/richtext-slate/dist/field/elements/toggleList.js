...
export const toggleList = (editor, format)=>{
    ...
    // Otherwise we can assume that we should just activate the list
        ...
        Transforms.setNodes(editor, {
            ...
            match: (node, path)=>{
                const match = Element.isElement(node) && typeof node.type === 'undefined' && path.length === parentNodePath.length + 1;
                return match;
            },
            voids: true
        });
        ...
        const nodesToWrap = Array.from(Editor.nodes(editor, {
            match: (node, path)=>{
                const match = Element.isElement(node) && typeof node.type !== 'undefined' && node.type !== 'li' && path.length === parentNodePath.length + 1;
                return match;
            }
        }));
