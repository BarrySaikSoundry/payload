...
export const generatePageMetadata = async ({ config: configPromise, params })=>{
    ...
    switch(segments.length){
        case 0:
            {
                meta = await generateDashboardMetadata({
                    config,
                    i18n
                });
                break;
        ...
        case 1:
            {
                if (oneSegmentMeta[segmentOne] && segmentOne !== 'account') {
                    ...
                    // --> /account
                    meta = await generateAccountMetadata({
                        config,
                        i18n
                    });
                    break;
        ...
        case 2:
            {
                if (segmentOne === 'reset') {
                    // --> /reset/:token
                    meta = await generateResetPasswordMetadata({
                        config,
                        i18n
                    });
                }
                if (isCollection) {
                    ...
                    meta = await generateListMetadata({
                        collectionConfig,
                        config,
                        i18n
                    });
        ...
        default:
            {
                if (segmentTwo === 'verify') {
                    ...
                    meta = await generateVerifyMetadata({
                        config,
                        i18n
                    });
